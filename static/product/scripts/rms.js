    ...
    function selectRoomNO(url, item, used) {
        ...
        $.sdpost(url, params, function (re) {
            if(re.code === 0){
                ...
                layer.alert("Falha ao obter dados da sala", {icon: 2, title: 'erro'})
            }
        });
    }
    ...
    getEditableParam = function (url, validate, refreshPk) {
        ...
        return {
            ...
            success: function (response, newValue) {
                if (response.code == 0) {
                    parent.layer.msg(response.msg, {icon: 1, title: 'Sucesso'});
                    if (refreshPk) {
                        //刷新列表数据
            ...
            error: function (response, newValue) {
                if (response.status === 500) {
                    return 'Erro do sistema, atualize e tente novamente';
                }
                if (response.status === 404) {
                    return 'Falha na solicitação, atualize e tente novamente';
                } else {
