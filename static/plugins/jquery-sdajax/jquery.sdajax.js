                ...
                error: function (XHR, msg, e) {
                    ...
                    if (typeof (XHR.responseText) !== 'undefined') {
                        if (XHR.responseText.indexOf('HttpRequestValidationException') > -1) {
                            win.layer.alert("Falha: " + 'Sua digitação contém caracteres malicisos: “&#” aguarde', { icon: 2, title: 'Erro' });
                        } else {
                            win.layer.alert("Falha: " + XHR.responseText, { icon: 2, title: 'Erro' });
                        }
                    } else {
                        win.layer.alert("Falha", { icon: 2, title: 'Erro' });
                    }
                }
            });
        },
        ...
        sdget: function (url, data, success, async, dataType) {
            ...
            $.ajax({
                ...
                error: function (XHR, msg, e) {
                    ...
                    if (typeof (XHR.responseText) !== 'undefined') {
                        if (XHR.responseText.indexOf('HttpRequestValidationException') > -1) {
                            win.layer.alert("Falha：" + '您输入的内容里有潜在危险的字符，例如：“&#” 等', { icon: 2, title: 'erro' });
                        } else {
                            win.layer.alert("Falha：" + XHR.responseText, { icon: 2, title: 'erro' });
                        }
                    } else {
