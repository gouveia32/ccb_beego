...
type Cliente struct {
	...
	PrecoBase     float64   `orm:"column(preco_base)" form:"PrecoBase"`
}

type ClienteQueryParam struct {
	BaseQueryParam
	Nome    string
	Contato string
	Estado  string
...
func ClienteTBName() string {
	return "cliente"
}

func ClientePageList(params *ClienteQueryParam) ([]*Cliente, int64) {
	query := orm.NewOrm().QueryTable(ClienteTBName())
	data := make([]*Cliente, 0)
...
func ClienteDataList(params *ClienteQueryParam) []*Cliente {
	params.Limit = -1
	params.Sort = "Id"
	params.Order = "asc"
	data, _ := ClientePageList(params)
	return data
}

func ClienteBatchDelete(ids []int) (int64, error) {
	query := orm.NewOrm().QueryTable(ClienteTBName())
	num, err := query.Filter("id__in", ids).Delete()
	return num, err
}

func ClienteOne(id int) (*Cliente, error) {
	o := orm.NewOrm()
	m := Cliente{Id: id}
	err := o.Read(&m)
	if err != nil {
